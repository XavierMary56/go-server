...
| 场景 | 推荐 | 理由 |
|-----|------|------|
| 用户实时发表评论 | 异步 | 快速响应，容量大，结果通过回调 |
| 后台批量审核 | 异步 | 并发高，不阻塞主进程 |
| 对重要内容的实时审核 | 同步 | 需要立即知道审核结果 |
| API 响应时间敏感 | 异步 | 立即返回 task_id，不等待审核 |
...
**Q: 我应该使用同步还是异步 API？**
A: 如果需要在返回响应前立即获得审核结果，使用同步。如果可以通过 Webhook 异步获得结果，使用异步（推荐，性能更好）。
